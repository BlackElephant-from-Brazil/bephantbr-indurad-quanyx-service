...
void ImageProcessing::convertGray162RGB(const unsigned char * input, unsigned char* output, int width, int height)
{
    const uint16_t *data = (const uint16_t *)input;
    for (int i = 0; i < (width*height); i++) {
        const uint16_t greyPixelValue = data[i];

        // LUT is formated fot 11 bit data
        output[3 * i] = _colorMapDisp[greyPixelValue][0];
        output[3 * i + 1] = _colorMapDisp[greyPixelValue][1];
        output[3 * i + 2] = _colorMapDisp[greyPixelValue][2];
    }
}

void ImageProcessing::convertGray82RGB(const unsigned char * input, unsigned char* output, int width, int height)
{
    const uint8_t *data = (const uint8_t *)input;
    for (int i = 0; i < (width*height); i++) {
        const uint8_t greyPixelValue = data[i];

        // LUT is formated fot 11 bit data
        // Take 1 value out of 4 to display 8 bit data
        output[3 * i] = _colorMapDisp[greyPixelValue*4][0];
        output[3 * i + 1] = _colorMapDisp[greyPixelValue*4][1];
        output[3 * i + 2] = _colorMapDisp[greyPixelValue*4][2];
    }
}
